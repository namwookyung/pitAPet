...
<!-- 상품 -->
	<section>
		<p style="text-align: left;"><c:set var="Item" value="<%=p_item %>"/>${fn:substring(Item,0,3) } > ${fn:substring(Item,3,6) }</p>
		<h4>${fn:substring(Item,0,3) } ${fn:substring(Item,3,6) }</h4>
		<c:if test="${prodItemList != null }">
			<div class="bdiv">
				<c:forEach var="prodItem" items="${prodItemList }" varStatus="status">
					<div class="card" id="card">
						<a href="prodView.prod?p_id=${prodItem.p_id }"> <img src="./img/${prodItem.p_image }" class="card-img-top" alt="...">
							<div class="card-body">
								<p class="card-text">
									<b>${prodItem.p_name }</b><br>
									<fmt:formatNumber value="${prodItem.p_price }" pattern="#,###,###" /> 원
								</p>
							</div>
						</a>
